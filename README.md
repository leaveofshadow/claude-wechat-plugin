...
之后手机发送任意指令即可操作。下班前 `/wx-off` 断开。

### 场景二：多项目切换

适合同时在多个项目间工作，通过微信远程切换。

```
# 在项目 A 目录启动
cd project-a && claude
/wx-on             # 连接微信，当前项目为 project-a

# 需要切换到项目 B
/wx-switch project-b   # 或在微信中发送 "切换 project-b"
```
...
适合需要远程审批工具执行的场景（如生产环境操作）。

```
/wx-approve on     # 开启审批模式
```

开启后 Claude 执行 Bash、Edit、Write 等操作前会发送微信审批请求，手机回复 `y`/`n` 即可控制。

```
/wx-approve off    # 关闭审批（默认状态）
```
